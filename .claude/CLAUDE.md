...
- `kraken` — headset **lighting** uses a different report protocol, so it does NOT go through `build`. `core.apply` routes `method == 'kraken'` to `_apply_kraken` → `protocol.build_kraken` (a 37-byte report-id-0x04 request report, no CRC, written to fixed RAM addresses as a HID **output** report via `transport.output_targets`/`write_output`, not a feature report). Addresses per pid live in `core.KRAKEN_LIGHTING` (Kylie: V2/TE/Ultimate/Kitty V2 = `0x172D`/`0x1741`; Rainie: 7.1 Chroma = `0x1008`/`0x15DE`; Classic = on/off + spectrum, no color). Supports static/off/spectrum/breathing. Ported from openrazer `razerkraken_driver`; not yet hardware-verified here. Kraken **EQ**/audio DSP is still host-side Synapse — not on-device.
