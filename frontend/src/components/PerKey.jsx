...
// Per-key / per-zone editor. `accent` is the app's current color (the paint).
// applyFrame(rows) and applyZone(led, rgb) do the HID work in the parent.
export default function PerKey({ currentPid, accent, applyFrame, applyZone }) {
  const t = useT();
  const matrix = matrixFor(currentPid);      // { rows, cols, guess?, demo? } | null
  const zones = zonesFor(currentPid);        // [{ id, name }] | null
  const layout = layoutFor(matrix);          // labeled keyboard layout, or null
  const rows = matrix?.rows || 0, cols = matrix?.cols || 0;
  ...
  // (re)build the grid whenever the matrix size changes
  useEffect(() => {
    if (!matrix) { setCells([]); return; }
    setCells(Array.from({ length: rows * cols }, () => base.slice()));
  }, [rows, cols]); // eslint-disable-line react-hooks/exhaustive-deps

  // stop painting when the pointer is released anywhere
  useEffect(() => {
  ...
  const hasGrid = !!matrix;
  if (!hasGrid && !zones)
    return <p className="text-[11px] text-neutral-500">{t("keysNoMatrix")}</p>;

  return (
    <div className="space-y-4">
      {zones && (
        <div>
          <p className="mb-1.5 text-[11px] text-neutral-500">{t("zonesTitle")}</p>
          <div className="flex flex-wrap gap-2">
            {zones.map((z) => (
              <button key={z.id} onClick={() => applyZone(z.id, accent)}
                      className="flex items-center gap-1.5 rounded-lg bg-neutral-800/80 border border-neutral-700 hover:border-emerald-500/60 px-2.5 py-1.5 text-xs font-semibold">
                <span className="h-3 w-3 rounded-sm ring-1 ring-white/20" style={{ background: rgbToHex(accent) }} />
                {z.name}
              </button>
            ))}
          </div>
